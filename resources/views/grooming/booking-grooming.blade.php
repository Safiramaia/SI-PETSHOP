<x-layout>
    <div class="flex justify-center items-center min-h-screen mt-2 w-full">
        <div class="w-full md:w-2/3 border border-gray-300 p-12 rounded-lg bg-white shadow-md dark:bg-gray-800 dark:border-gray-600">
            <h2 class="mb-4 mt-2 text-xl font-bold text-center text-gray-900 dark:text-white">Form Pendaftaran Grooming</h2>

            <form id="groomingForm" action="{{ route('grooming.store') }}" method="POST" onsubmit="showOrderDetails(event)">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <select name="id_jenis" id="id_jenis"
                        ...
                        <option value="" disabled {{ old('id_jenis') ? '' : 'selected' }}>Pilih jenis grooming
                        </option>
                        @foreach ($jenisGrooming as $jenis)
                            <option value="{{ $jenis->id }}"
                                {{ old('id_jenis') == $jenis->id || $selectedJenisId == $jenis->id ? 'selected' : '' }}>
                                {{ $jenis->nama_jenis }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="durasi" class="form-input w-full rounded-lg bg-gray-50 dark:bg-gray-700"
                        value="{{ old('durasi', $durasi) }}" readonly>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="datetime-local" name="tanggal_booking" id="tanggal_booking"
                        class="form-input w-full rounded-lg bg-gray-50 dark:bg-gray-700" required
                ...
                <div class="flex justify-end gap-4 mt-4">
                    <a href="{{ route('grooming.index') }}"
                        class="bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 text-white font-medium rounded-lg px-5 py-2.5 hover:bg-gray-900">Kembali</a>
                    <button type="submit"
                        class="bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 text-white font-medium rounded-lg px-5 py-2.5 hover:bg-blue-800">Booking</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function confirmOrder() {
            document.getElementById('groomingForm').submit();
        }
    </script>
</x-layout>
