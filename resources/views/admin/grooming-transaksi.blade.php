    ...
    <div class="mx-4 lg:mx-10 max-w-screen-2xl mt-8">
        ...
        <div class="overflow-x-auto relative sm:rounded-lg mt-5 shadow-md dark:bg-gray-800">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($transaksi_grooming as $index => $transaksis)
                        <tr
                            ...
                            <form id="delete-form-{{ $transaksis->id }}"
                                action="{{ route('grooming-transaksi.destroy', $transaksis->id) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
            ...
            <div class="flex justify-between items-center px-4 py-3 bg-white border-t border-gray-200 dark:bg-gray-800">
                <div class="text-sm text-gray-800 dark:text-white font-bold">
                    Showing {{ $transaksi_grooming->firstItem() ?? 0 }} to {{ $transaksi_grooming->lastItem() ?? 0 }} of
                    {{ $transaksi_grooming->total() }} data
                </div>
                <div>
                    {{ $transaksi_grooming->links('components.pagination') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById("simple-search").addEventListener("keyup", function() {
            ...
            rows.forEach(function(row) {
                var IdUser = row.querySelector("td:nth-child(2)").textContent.toLowerCase();
                var metodePembayaran = row.querySelector("td:nth-child(3)").textContent.toLowerCase();

                if (IdUser.includes(value) || metodePembayaran.includes(value)) {
                    row.style.display = "";
                    found = true;
                } else {
