<x-layout_admin>
    <div class="mx-4 lg:mx-10 max-w-screen-2xl mt-8">
                    ...
                    <div id="tambahModal" tabindex="-1" aria-hidden="true"
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <form action="{{ route('jenis-grooming.store') }}" method="POST" class="p-4 md:p-5"
                                        ...
                                        <div class="col-span-2">
                                            ...
                                            <div class="flex items-center">
                                                <input type="number" name="harga" id="harga"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                                    placeholder="Contoh : 80000" required
                                                    pattern="^\d+([,.]\d{1,2})?$" />
                                            </div>
                                        </div>
                                        <div class="col-span-2">
        ...
        <div class="overflow-x-auto relative sm:rounded-lg mt-5 shadow-md dark:bg-gray-800">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($jenisGrooming as $index => $jenis)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-4 py-2 text-center border-r border-gray-200">{{ $index + 1 }}</td>
                            <td class="px-4 py-2 border-r border-gray-200">{{ $jenis->nama_jenis }}</td>
                            <td class="px-4 py-2 border-r border-gray-200 flex justify-center items-center">
                                @if ($jenis->foto)
                                    <img src="{{ asset('jenis_grooming/' . $jenis->foto) }}" alt="{{ $jenis->nama_jenis }}"
                                        class="w-20 h-20 object-cover">
                                @else
                                    <span>Tidak ada foto</span>
                                @endif
                            </td>
                            <td class="px-4 py-2 border-r border-gray-200">{{ $jenis->deskripsi }}</td>
                            <td class="px-4 py-2 text-center border-r border-gray-200">Rp
                                {{ number_format($jenis->harga, 2, ',', '.') }}
                            </td>
                            <td class="px-4 py-2 text-center border-r border-gray-200">{{ $jenis->durasi }} mnt</td>
                            <td class="px-4 py-2 text-center border-r border-gray-200">
                                <!-- Modal Edit -->
                                <button data-modal-target="editModal{{ $jenis->id }}"
                                    data-modal-toggle="editModal{{ $jenis->id }}"
                                ...
                                <!-- Main Modal Edit -->
                                <div id="editModal{{ $jenis->id }}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            <div
                                                class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Jenis
                                                    Grooming</h3>
                                            </div>
                                            <form id="editForm" action="{{ route('jenis-grooming.update', $jenis->id) }}"
                                                ...
                                                <div class="grid gap-4 mb-4">
                                                    ...
                                                    <div class="col-span-2 flex">
                                                        ...
                                                        <textarea id="deskripsi" name="deskripsi" rows="4"
                                                            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $jenis->deskripsi }}</textarea>
                                                    </div>
                                                    <div class="col-span-2 flex">
                                                        <label for="harga"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white mr-4 w-1/3">Harga</label>
                                                    ...
                                                    <div class="col-span-2 flex">
                                                        <label for="durasi"
                                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white mr-4 w-1/3">Durasi</label>
                                                        <div class="flex items-center w-full">
                                                ...
                                                <div class="flex justify-end mt-4">
                                                    <button data-modal-hide="editModal{{ $jenis->id }}" type="submit"
                                                    ...
                                                    <button onclick="closeModal('editModal{{ $jenis->id }}')" type="button"
                                                        ...
                                                        aria-label="Batal" aria-describedby="close-modal-description">
                                                        Batal
                                                    </button>
                                                </div>
                                            </form>
                                ...
                                <!-- Tombol Hapus -->
                                <button onclick="openDeleteModal('{{ $jenis->id }}')"
                                        ...
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <h3 class="mb-5 text-lg text-gray-500 dark:text-gray-400">
                                                    Apakah Anda yakin ingin menghapus jenis grooming ini?
                                                </h3>

                                                <form id="deleteForm"
                                                    action="{{ route('jenis-grooming.destroy', $jenis->id) }}" method="POST"
            ...
            <div class="flex justify-between items-center px-4 py-3 bg-white border-t border-gray-200 dark:bg-gray-800">
                <div class="text-sm text-gray-800 dark:text-white font-bold">
                    Showing {{ $jenisGrooming->firstItem() ?? 0 }} to {{ $jenisGrooming->lastItem() ?? 0 }} of
                    {{ $jenisGrooming->total() }} data
                </div>
                <div>
                    {{ $jenisGrooming->links('components.pagination') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        function openEditModal(id, namaJenis, deskripsi, harga, durasi, foto) {
            ...
            editForm.action = `/admin/jenis-grooming/${id}`;
        }
        ...
        function openDeleteModal(id) {
            ...
            form.action = `/admin/jenis-grooming/${id}`;
    ...
    <script>
        document.getElementById("simple-search").addEventListener("keyup", function () {
            ...
            rows.forEach(function (row) {
                var namaJenis = row.querySelector("td:nth-child(2)").textContent.toLowerCase();
                if (namaJenis.includes(value)) {
                    row.style.display = "";
                    found = true;
                } else {
