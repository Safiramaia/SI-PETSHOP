    ...
    <div class="mx-4 lg:mx-10 max-w-screen-2xl mt-8">
        ...
        <div class="overflow-x-auto relative sm:rounded-lg mt-5 shadow-md dark:bg-gray-800">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($grooming as $data_grooming)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 
                            hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-4 py-2 text-center text-black border-r border-gray-200">
                                {{ $data_grooming->nama_kucing }}
                            </td>
                            <td class="px-4 py-2 text-center text-black border-r border-gray-200">
                                {{ number_format($data_grooming->berat, 0) }} kg
                            </td>                            
                            <td class="px-4 py-2 text-center text-black border-r border-gray-200">
                                {{ $data_grooming->umur }} thn
                            </td>
                            <td class="px-4 py-2 text-black border-r border-gray-200">
                                {{ $data_grooming->jenisGrooming->nama_jenis ?? 'Tidak Ada Jenis' }}
                            </td>
                            <td class="px-4 py-2 text-center text-black border-r border-gray-200">
                                {{ (new DateTime($data_grooming->tanggal_booking))->format('d-m-Y H:i') }}
                            </td>
                            <td class="px-4 py-2 text-center text-black border-r border-gray-200">
                                {{ $data_grooming->transaksi->id ?? 'Dibatalkan' }}
                            </td>
                            <td class="px-4 py-2 text-black border-r border-gray-200">
                                Rp {{ number_format($data_grooming->harga_total, 2, ',', '.') }} 
                            </td>
                            <td class="px-4 py-2 text-center text-black border-r border-gray-200">
                                {{ (new DateTime($data_grooming->transaksi->tanggal_transaksi ?? ''))->format('d-m-Y H:i') }}
                            </td>
                        </tr>
                    @endforeach
                    @if($grooming->isEmpty())
                        <tr>
                            <td colspan="8" class="text-center text-gray-500">
                                Tidak ada data transaksi grooming
                            </td>
                        </tr>
                    @endif
                </tbody>
            ...
            <div class="flex justify-between items-center px-4 py-3 bg-white border-t border-gray-200 dark:bg-gray-800">
                <div class="text-sm text-gray-800 dark:text-white font-bold">
                    Menampilkan {{ $grooming->count() }} data grooming
                </div>
                <div>
                    {{ $grooming->links('components.pagination') }}
                </div>
            </div>
        </div>
        ...
        <script>
            document.getElementById("simple-search").addEventListener("keyup", function() {
                ...
                rows.forEach(function(row) {
                    ...
                    var namaProduk = row.querySelector("td:nth-child(3)").textContent.toLowerCase();
